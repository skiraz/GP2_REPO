...
class PepperEnv(gymnasium.Env):    
    def __init__(self,gui = True):
        ...
        self.observation_space = spaces.Dict({
            "kinematics" : spaces.Box(low = np.array(lower_limits),
                                        high = np.array(upper_limits)),
    ...
    def _setupScene(self):
        ...
        self.table_position = [0.6, 0.3, .9  ]  # Modify this position to place the table where you want
    ...
    def _getState(self, convergence_norm=0.15):
        ...
        #     self.episode_over = True
        if (time.time() - self.episode_start_time) > 3:
            reward += -1
            self.episode_over = True
            # self.episode_failed = True
    ...
    def _resetScene(self):
        ...
        # random_num = random.uniform(0.06, 0.22)

        self.soda_pos = [0.315, 0.04, .96]
...
if __name__ == "__main__":
    ...
    model = PPO2.load(os.path.join(env.models_dir,"iter 3"),env=env)
    # model = PPO2(  
    #     "MultiInputPolicy",
    #     env,
    #     # buffer_size=100,
    ...
    model.save(os.path.join(env.models_dir,"iter 7"))
