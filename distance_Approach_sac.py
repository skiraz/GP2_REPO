...
class PepperEnv(gymnasium.Env):    
    ...
    def contact_points(self):
        finger1_link_index = 36
        finger2_link_index = 37
        finger3_link_index = 34
        palm_link_index = 35

        # Get the collision shape ID or link index for the object
        object_link_index = 0

        # Perform collision check
        contact_points = p.getContactPoints(bodyA=self.robot_body_id, linkIndexA=finger1_link_index, bodyB=self.soda, linkIndexB=object_link_index)
        contact_points += p.getContactPoints(bodyA=self.robot_body_id, linkIndexA=finger2_link_index, bodyB=self.soda, linkIndexB=object_link_index)
        contact_points += p.getContactPoints(bodyA=self.robot_body_id, linkIndexA=finger3_link_index, bodyB=self.soda, linkIndexB=object_link_index)
        contact_points += p.getContactPoints(bodyA=self.robot_body_id, linkIndexA=palm_link_index, bodyB=self.soda, linkIndexB=object_link_index)
        if len(contact_points) > 0:
            print("Collision detected between the left hand and the object!")
            print(contact_points)
        else:
            print("No collision detected.")

 
        self.contact_points_num += len(contact_points)
    ...
    def _getObservation(self):
        ...
        obs = {"kinematics":kinematics,"soda_can":[self.x,self.y],"image":self.img}
    ...
    def camera_view(self,robot):
        ...
        for i in range(1):
            ...
            try :
                self.x = pred["predictions"][0]["x"]
                self.y = pred["predictions"][0]["y"]
